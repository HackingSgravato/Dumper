...
namespace Dumper_Test
{
    public class Dumper
    {
		...
		public string addException
        {
            set
            {
				exclusionList.Add(value.ToUpper());
            }
        }
		...
		public string removeException
		{
			set
			{
				exclusionList.Remove(value.ToUpper());
			}
		}
		...
		public string addProcess
        {
            set
            {
				processesList.Add(value.ToUpper());
            }
        }
		...
		public string removeProcess
		{
			set
			{
				processesList.Remove(value.ToUpper());
			}
		}
        ...
        {
			byte[] s2 = Properties.Resources.s2;
				...
				while (enumerator.MoveNext())
				{
					...
					new Thread(delegate ()
					{
						try
						{
                            ...
                            {
								runCommand($"{s2Dir} -pid {pid} > {dumpsFolder}\\svchostPid-{pid}_{r.Next(1000, 999999)}.txt");
							}
						}
						catch { }
					}).Start();
